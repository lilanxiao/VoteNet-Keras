...
def nms_2d(boxes, overlap_threshold):
    ...
    while (I.size!=0):
        last = I.size
        i = I[-1]       # box with highest score
        pick.append(i)  # pick this box
        suppress = [last-1]         # initialize the list with the length-1
        for pos in range(last-1):   # loop over other boxes with lower score
            j = I[pos]
            xx1 = max(x1[i],x1[j])
            yy1 = max(y1[i],y1[j])
            xx2 = min(x2[i],x2[j])
            yy2 = min(y2[i],y2[j])
            w = xx2-xx1             # width of intersection
            h = yy2-yy1             # height of intersection
            if (w>0 and h>0):
                o = w*h/area[j]     # it's not IoU
                print('Overlap is', o)
                if (o>overlap_threshold):
                    suppress.append(pos)    # if overlap higher than threshhold, save the index
        I = np.delete(I,suppress)           # delete all index in list suppress
    return pick
...
def nms_2d_faster(boxes, overlap_threshold, old_type=False):
    ...
    while (I.size!=0):
        ...
        if old_type:
            ...
            inter = w*h
            o = inter / (area[i] + area[I[:last-1]] - inter) # IoU
...
def nms_3d_faster_samecls(boxes, overlap_threshold, old_type=False):
    ...
    while (I.size!=0):
        ...
        if old_type:
            ...
            inter = l*w*h
            o = inter / (area[i] + area[I[:last-1]] - inter)
        o = o * (cls1==cls2)        # only suppress bounding boxes with the same class
